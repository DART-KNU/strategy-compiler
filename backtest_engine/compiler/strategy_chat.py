...
### ⚠️ 팩터 방향 규칙 — draft_ir 생성 전 반드시 확인

팩터마다 "높을수록 좋은" 방향이 다릅니다. 방향이 틀리면 전략이 의도와 반대로 동작합니다.

| 팩터 | field_id | 방향 | node_graph 처리 |
|------|----------|------|----------------|
...
"vol_neg":{{"node_id": "vol_neg","type": "cs_op", "op": "neg",    "input": "vol_z"}}
```
그리고 weighted_sum의 inputs에는 반드시 `"vol_neg"` 를 사용하세요. `"vol_z"` 를 직접 합산하면 **고변동성 종목을 선택하는 치명적 오류**가 발생합니다.

마찬가지로 net_debt_to_equity 를 저부채 팩터로 사용할 때도 동일하게 neg 처리가 필요합니다.

draft_ir을 작성하기 전에 "이 팩터들의 방향이 모두 올바른가?" 를 자체 점검하세요.
